...
> ⚠️ **これは [isuzu-shiranui/UnityMCP](https://github.com/isuzu-shiranui/UnityMCP) のカスタムフォークです。**
...
このフォークでは以下の改善を加えています：

### ✨ 新機能: コンパイル制御ツール
- **`compile.refresh`** − Unity のスクリプトコンパイルを手動トリガー
- **`compile.getStatus`** − コンパイル状態をリアルタイム監視
  - コンパイル進行状態
  - エラー・警告の詳細取得
  - コード実行前の準備完了確認

### 🔧 安定性向上
- **問題のあった .NET Core DLL を削除**
  - `System.Private.CoreLib.dll`
  - `System.Runtime.Loader.dll`
  - Unity パッケージ環境での conflicts を解決

### 📦 インストール簡素化
- `manifest.json` への 1 行追記で自動インストール可能
- 詳細は [INSTALLATION.md](./INSTALLATION.md) 参照
